...
pub fn draw_hud(assets: &Assets, run: &Run, gold_display: u32, show_timer: bool, view_w: f32, _view_h: f32) {
    let y = 30.0;
    ...
    draw_text(&format!("Time: {}", fmt_time(run.level.elapsed())), 14.0, 122.0, 22.0, LIGHTGRAY);
    ...
    if show_timer {
        if let Some(e) = run.level.active_effect {
            let name = match e.kind {
                ConsumableKind::SuperPick => "Super Pick",
                ConsumableKind::StickySmell => "Sticky Smell",
            };
            draw_text(&format!("{name}: {:.1}s", e.remaining.max(0.0)), 14.0, 150.0, 20.0, YELLOW);
        }
    }
}
